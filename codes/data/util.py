...
def calculate_weights_indices(in_length, out_length, scale, kernel, kernel_width, antialiasing):
    ...
    print("indices:{}, shape:{}".format(indices, indices.shape))

    # The weights used to compute the k-th output pixel are in row k of the
    # weights matrix.
    distance_to_center = u.view(out_length, 1).expand(out_length, P) - indices
    print("distance_to_center:{}j, shape:{}".format(distance_to_center, distance_to_center.shape))
    # apply cubic kernel
    if (scale < 1) and (antialiasing):
    ...
    print("weight_zero_tmp:{}".format(weights_zero_tmp))
...
def imresize(img, scale, antialiasing=True):
    ...
    print("in_H:{}, in_W:{}, scale:{}".format(in_H, in_W, scale))
    ...
    print("weight_H:{}".format(weights_H))
    print("indices_H:{}".format(indices_H))
    print("sym_len_Hs:{}".format(sym_len_Hs))
    print("sym_len_He:{}".format(sym_len_He))
    # process H dimension
    # symmetric copying
    img_aug = torch.FloatTensor(B*in_C, in_H + sym_len_Hs + sym_len_He, in_W).to(device)
    img_aug.narrow(1, sym_len_Hs, in_H).copy_(img)  # narrow(dimension, start, length) -> Tensor (start+length-1)
