...
def random_batch_kernel(
    ...
    if rate_iso == 1:
        ...
        xx = xx[None].repeat(batch, 0)   # Here is numpy's  form,if the form is tensor shuold be xx[None].repeat(batch, 1, 1)
...
class SRMDPreprocessing(object):
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        # Adding by fu
        # kernel: B x C x k_l x k_l
        # kernel_code = b_kernels.contiguous().view((B, 1, self.l, self.l)).repeat(1, C, 1, 1)
        # print('kernel_code.shape:', kernel_code.shape)

        # Down sample
        if self.scale != 1:
            # lr_blured_t = b_Bicubic(hr_blured_var, self.scale)
            # Use F.conv2d of 1x1 kernel and sf stride for downsample
            # kernel_one = torch.ones(B, C, 1, 1).to(device)
            # hr_blured_var = hr_blured_var.view((1, C*B, H, W))
            # kernel_one_var = (
            #     kernel_one.contiguous().view(B*C, 1, 1, 1)
            # )
            # lr_blured_t = F.conv2d(hr_blured_var, kernel_one_var, stride=self.scale, groups=B*C).view((B, C, H//2, W//2))
            # lr_blured_t = F.conv2d(hr_blured_var, kernel_one_var, stride=self.scale, groups=B*C).view((B, C, H//4, W//4))

            lr_blured_t = hr_blured_var[:, :, ::self.scale, ::self.scale]
        else:
        ...
        if self.noise:
            ...
            noise_level = torch.rand(B,1,1,1).to(lr_blured_t.device) * (self.noise_high/255.0) if self.random else (self.noise_high/255.0)
            noise = noise_level * torch.randn_like(lr_blured_t)
            lr_noised_t = torch.clamp(torch.add(lr_blured_t, noise), min=0.0, max=1.0)
        else:
        ...
        #   Fu: delet return( re_code )
        return (lr_re, b_kernels) if kernel else (lr_re, b_kernels)
