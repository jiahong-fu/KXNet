...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        # define network and load pretrained models, find_unused_parameters=True
        self.netG = networks.define_G(opt).to(self.device)
        if opt["dist"]:
            self.netG = DistributedDataParallel(
                self.netG, device_ids=[torch.cuda.current_device()], find_unused_parameters=True
            )
        else:
    ...
    def feed_data(self, LR_img, GT_img=None, kernel=None, scale=None,):
        self.var_L = LR_img.to(self.device)
        if not (GT_img is None):
            self.real_H = GT_img.to(self.device)
        if not (kernel is None):
            self.real_ker = kernel.to(self.device)
    ...
    def optimize_parameters(self, step):
        ...
        # total_loss += 0.1 * self.cri_pix(srs_init[-1], self.real_H)
        # self.log_dict["x_init"] = (self.cri_pix(srs_init[-1], self.real_H)).item()

        # self.cri_pix_ker = nn.MSELoss().to(self.device)
        for ind in range(len(kernels) - 1):
            d_kr = self.cri_pix(kernels[ind], self.real_ker)
        ...
        ind = len(kernels) - 1
        d_kr = self.cri_pix(kernels[-1], self.real_ker)
        d_sr = self.cri_pix(srs[-1], self.real_H)
        self.log_dict["l_pix%d" % ind] = d_sr.item()
        self.log_dict["l_ker%d" % ind] = d_kr.item()
        total_loss +=  d_kr
        total_loss += d_sr
        ...
        ######### Add to return the all results ##############
        # return (r_loss, srs, kernels)
